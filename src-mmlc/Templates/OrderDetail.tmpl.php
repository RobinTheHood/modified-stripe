...
<div class="rth-stripe-content">
                            ...
                            <div class="rth-stripe-property-list-item-value">
                                <?php foreach ($charge->refunds->data as $refund) : ?>
                                    <div style="margin-bottom: 8px;">
                                        ...
                                        <div><strong>Betrag:</strong> <?= $view->formatAmount($refund->amount, $refund->currency); ?></div>
                                        <div><strong>Status:</strong> <?= $refund->status; ?></div>
                                        <div><strong>Datum:</strong> <?= $view->formatTimestamp($refund->created); ?></div>
                                        <?php if (!empty($refund->reason)) : ?>
                                            <div><strong>Grund:</strong> <?= $refund->reason; ?></div>
                                        <?php endif; ?>
                                    </div>
                                <?php endforeach; ?>
                            </div>
    ...
    <div class="rth-stripe-actions">
        <?php if ('requires_capture' === $paymentIntent->status) : ?>
            <a href="/rth_stripe.php?action=capture&order_id=<?= $_GET['oID']; ?>&payment_intent_id=<?= $paymentIntent->id; ?>" class="rth-stripe-button">
        ...
        <?php if (in_array($paymentIntent->status, ['succeeded', 'processing']) && (!$paymentIntent->latest_charge || !$charge->refunded)) : ?>
            <a href="/rth_stripe.php?action=refund&order_id=<?= $_GET['oID']; ?>&payment_intent_id=<?= $paymentIntent->id; ?>" class="rth-stripe-button" style="background-color: #ef3b5a;">
                Rückerstattung durchführen
            </a>
        <?php endif; ?>

        <?php if ('canceled' !== $paymentIntent->status && !in_array($paymentIntent->status, ['succeeded', 'processing'])) : ?>
            <a href="/rth_stripe.php?action=cancel&order_id=<?= $_GET['oID']; ?>&payment_intent_id=<?= $paymentIntent->id; ?>" class="rth-stripe-button" style="background-color: #6c757d;">
                Zahlung stornieren
            </a>
        <?php endif; ?>
    </div>
